...
void FlashOptionData(uint8_t d0, uint8_t d1) {
    ...
    // The entire 64 byte data block of the "User-selected words" will
    // be erased so we need to keep a copy of the content for re-writing
    // after erase. Save a few (20) bytes code space by moving 32 bits
    // at a time. 			hold[0]=OB->RDPR;
    // hold[1]=OB->USER; hold[2]=data0;
    // hold[3]=data1; 			hold[4]=OB->WRPR0;
    // 			hold[5]=OB->WRPR1;
    uint32_t *hold32p = (uint32_t *)hold;
    uint32_t *ob32p = (uint32_t *)OB_BASE;
    hold32p[0] = ob32p[0];        // Copy RDPR and USER
    hold32p[1] = d0 + (d1 << 16); // Copy in the two Data values to be written
    ...
    FLASH->CTLR |= CR_STRT_Set;  // STRT RW1 Start. Set 1 to start an erase
    // action,hw automatically clears to 0
    ...
    FLASH->CTLR |=
        CR_OPTPG_Set; // OBG  RW Perform user-selected word programming
